...
Missing data, highly correlated data and any columns with values near zero are removed from the dataset since they add very little predictive value. Since this model is only focused on accelerometer data, other fields that do not measure that type of data have also been eliminated.
...
Since random forests can lead to overfitting, cross validation is used to determine a better estimate the of out of sample error by running the model on smaller subsets of the training dataset. The number of folds was set to 10 for this cross validation exercise with average error printed below. This resulted in projecting an 84% accuracy rate for out-of-bag predictions, which is quite a bit lower than the model's estimation of 99.23%.
...
These plots demonstrate the accuracy of the model and the order of importance of the variables used in creating the model.
